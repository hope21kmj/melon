...
#if defined(NDEBUG)
# error "Meloncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc0c57ea2f350d662ef8039eb2c35c95bcd1742c527583c9e5a5d6fd711d2160a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Meloncoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Meloncoin Signed Message:\n";
...
{
    // Meloncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetSpacing = 30; // Meloncoin: 1 minute block
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    static const int64 BlocksTargetSpacing = 30; // 0.5 Minute
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Meloncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Meloncoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9b787738c581c56ea7c2f125f4def359638f050d15c14783adc3bdb6c6447010");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "On December 6, 2018, Tottenham's SON scored his 100th goal European";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce   = 222705085;

        if (fTestNet)
        {
            block.nTime    = 1546506822;
            block.nNonce   = 1975299;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xba1945124412bcca7f574779e51357ddd213fc47dd10832e08e7fb36b9140af5"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x9a, 0xa8, 0x8b, 0xb7 }; // Meloncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MeloncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MeloncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MeloncoinMiner(CWallet *pwallet)
{
    printf("MeloncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("meloncoin-miner");
    ...
    try { loop {
        ...
        printf("Running MeloncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MeloncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MeloncoinMiner, pwallet));
}
